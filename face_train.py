...
def train(train_dir, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False):
    ...
    # Training directory
    print("code start")
    train_dir = os.listdir('TrainigData/')
    print(train_dir)

    # Loop through each person in the training directory
    for person in train_dir:
        pix = os.listdir("TrainigData/" + person)
    
        # Loop through each training image for the current person
        for person_img in pix:
            # Get the face encodings for the face in each image file
           
            face = face_recognition.load_image_file("TrainigData/" + person + "/" + person_img)
            print(face.shape)
...
if __name__ == "__main__":
    ...
    classifier = train("TrainigData/", model_save_path="TrainedModel/trained_knn_model.clf", n_neighbors=2)
