...
def upload_image():
    if request.method == "GET":
        ...
        start = time.time()
        results = []
        with open("TrainedModel/trained_knn_model.clf", 'rb') as f:
...
if __name__ == '__main__':
    ...
    #app.run(port=8352, host=" 172.16.0.231", debug=False, threaded=False, processes=3)
    app.run(port=11000, host="127.0.0.1", debug=True)
    #app.run(host=config.host_ip, port=config.host_port)